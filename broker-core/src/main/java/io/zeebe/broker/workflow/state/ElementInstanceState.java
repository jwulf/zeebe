...
public class ElementInstanceState {
  ...
  public ElementInstanceState(ZeebeDb<ZbColumnFamilies> zeebeDb) {
    ...
    recordColumnFamily =
        zeebeDb.createColumnFamily(ZbColumnFamilies.RECORDS, recordKey, storedRecord);
    ...
    recordParentChildColumnFamily =
        zeebeDb.createColumnFamily(
            ZbColumnFamilies.RECORD_PARENT_CHILD, recordParentStateRecordKey, DbNil.INSTANCE);
  ...
  public boolean isEmpty() {
    ...
    recordColumnFamily.whileTrue(
        (k, v) -> {
          isEmpty.compareAndSet(true, false);
          return false;
        });

    recordParentChildColumnFamily.whileTrue(
        (k, v) -> {
          isEmpty.compareAndSet(true, false);
          return false;
        });
